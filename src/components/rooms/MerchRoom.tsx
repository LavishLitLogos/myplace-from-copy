...
import { ShoppingBag, ExternalLink, Star, Plus, Trash2, Link } from 'lucide-react';
...
export function MerchRoom({ onNavigate }: MerchRoomProps) {
  ...
  const [form, setForm] = useState({ name: '', description: '', price_cents: '', image_url: '', button_label: 'Shop Now', button_url: '' });
  ...
  async function load() {
    setLoading(true);
    const { data } = await supabase.from('merch_products').select('*').eq('creator_id', profile!.id).order('is_featured', { ascending: false }).order('sort_order');
    setProducts(data ?? []);
    setLoading(false);
  }
  ...
  async function addProduct() {
    ...
    setSaving(false); setAddOpen(false); setForm({ name: '', description: '', price_cents: '', image_url: '', button_label: 'Shop Now', button_url: '' }); load();
  }
    ...
    <div className="pb-28">
          ...
          <div className="space-y-3">
            {featured.map(p => (
              <div key={p.id} className="relative rounded-2xl overflow-hidden" style={{ border: `1px solid ${accent}30`, background: `${accent}08` }}>
                ...
                {isCreator && <div className="absolute top-2 right-2 flex gap-1"><button onClick={() => toggleFeature(p.id, p.is_featured)} className={`w-6 h-6 rounded-full flex items-center justify-center ${p.is_featured ? 'text-yellow-400' : 'text-white/30'}`} style={{ background: 'rgba(0,0,0,0.5)' }}><Star size={10} /></button><button onClick={() => deleteProduct(p.id)} className="w-6 h-6 rounded-full flex items-center justify-center text-red-400/50 hover:text-red-400" style={{ background: 'rgba(0,0,0,0.5)' }}><Trash2 size={10} /></button></div>}
              </div>
            ))}
          </div>
        </Bin>
      ...
      {withLinks.length > 0 && (
        <Bin label="External Store Links" icon={Link} accent={accent}>
          <div className="grid grid-cols-2 gap-3">
            {withLinks.map(p => <ProductCard key={p.id} product={p} accent={accent} isCreator={isCreator} onFeature={() => toggleFeature(p.id, p.is_featured)} onDelete={() => deleteProduct(p.id)} />)}
          </div>
        </Bin>
      )}
      ...
      {withoutLinks.length > 0 && (
        <Bin label="Products" icon={ShoppingBag} accent={accent}>
          <div className="grid grid-cols-2 gap-3">
            {withoutLinks.map(p => <ProductCard key={p.id} product={p} accent={accent} isCreator={isCreator} onFeature={() => toggleFeature(p.id, p.is_featured)} onDelete={() => deleteProduct(p.id)} />)}
          </div>
        </Bin>
      )}
...
function ProductCard({ product, accent, isCreator, onFeature, onDelete }: { product: MerchProduct; accent: string; isCreator: boolean; onFeature: () => void; onDelete: () => void }) {
  return (
    <div className="relative rounded-2xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
      ...
      {isCreator && <div className="absolute top-2 right-2 flex gap-1"><button onClick={onFeature} className={`w-6 h-6 rounded-full flex items-center justify-center ${product.is_featured ? 'text-yellow-400' : 'text-white/30'}`} style={{ background: 'rgba(0,0,0,0.5)' }}><Star size={10} /></button><button onClick={onDelete} className="w-6 h-6 rounded-full flex items-center justify-center text-red-400/50 hover:text-red-400" style={{ background: 'rgba(0,0,0,0.5)' }}><Trash2 size={10} /></button></div>}
    </div>
  );
}
