...
interface AudioUploadProps {
  value: string;
  onChange: (url: string, durationSecs?: number) => void;
  label?: string;
  creatorId: string;
  maxSizeMB?: number;
}
...
export function AudioUpload({
  value, onChange, label = 'Audio', creatorId, maxSizeMB = 20.5,
}: AudioUploadProps) {
  ...
  const detectDuration = useCallback((url: string) => {
    const audio = new Audio();
    audio.src = url;
    audio.addEventListener('loadedmetadata', () => {
      const dur = Math.round(audio.duration);
      if (dur > 0 && isFinite(dur)) onChange(url, dur);
      else onChange(url);
    });
    audio.addEventListener('error', () => onChange(url));
  }, [onChange]);
  ...
  async function handleUpload(file: File) {
    ...
    setUploading(true);
    const path = storagePath(creatorId, 'audio', file.name);
    const url = await uploadFile(file, path);
    setUploading(false);
    if (url) detectDuration(url);
  ...
  return (
    <div>
      ...
      {mode === 'url' ? (
        ...
        <div>
          <input ref={fileRef} type="file" accept="audio/*" onChange={e => { const f = e.target.files?.[0]; if (f) handleUpload(f); }} className="hidden" />
          <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading}
            ...
            {uploading ? <div className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin" /> : <><Music size={14} /> Choose Audio File</>}
          </button>
        </div>
      )}
      {error && <p className="text-red-400 text-xs mt-1">{error}</p>}
      {value && <p className="text-white/30 text-[10px] mt-1 truncate">{value}</p>}
    </div>
  );
}
