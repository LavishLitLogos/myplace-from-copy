...
export function FileUpload({
  value, onChange, label, placeholder = 'https://...', creatorId, type = 'files', maxSizeMB = 15.5, accept,
}: FileUploadProps) {
  ...
  return (
    <div>
      ...
      {mode === 'url' ? (
        ...
        <div>
          <input ref={fileRef} type="file" accept={accept} onChange={e => { const f = e.target.files?.[0]; if (f) handleUpload(f); }} className="hidden" />
          <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading}
            ...
            {uploading ? <div className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin" /> : <><File size={14} /> Choose File</>}
          </button>
        </div>
      )}
